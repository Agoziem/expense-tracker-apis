...
role_checker = RoleChecker(["super_admin", "admin", "user"])
...
async def create_user_Account(
    ...
    content = EmailRawHTMLContent(
        ...
        html_content=f"<p>Welcome {new_user.first_name},</p><p>Thank you for registering. Please use the Code to verify your email <strong>{token_data.token}</strong>.</p><p>This code will expire in 24 hours.</p>",
        sender_name=settings.SENDER_NAME,
    )
    background_tasks.add_task(send_resend_email, recipients, content)
...
async def resend_verification_email(
    ...
    content = EmailRawHTMLContent(
        subject="Verify Your email",
        html_content=f"<p>Welcome {user_data.first_name},</p><p>Thank you for registering. Please use the Code to verify your email <strong>{token_data.token}</strong>.</p><p>This code will expire in 24 hours.</p>",
        sender_name=settings.SENDER_NAME,
    )
    background_tasks.add_task(send_resend_email, recipients, content)
...
async def login_users(
    ...
    if user and verify_password(password, user.password_hash):
        ...
        if not user.is_verified:
            ...
            content = EmailRawHTMLContent(
                subject="Verify Your email",
                html_content=f"<p>Welcome {user.first_name},</p><p>Thank you for registering. Please use the Code to verify your email <strong>{token_data.token}</strong>.</p><p>This code will expire in 24 hours.</p>",
                sender_name="Expense Tracker App",
            )
            background_tasks.add_task(send_resend_email, recipients, content)
        ...
        if user.two_factor_enabled:
            ...
            content = EmailRawHTMLContent(
                subject="2FA Code",
                html_content=f"<p>Your 2FA code is: <strong>{two_factor_token.token}</strong></p><p>This code will expire in 10 minutes.</p>",
                sender_name="Expense Tracker App",
            )
            background_tasks.add_task(send_resend_email, recipients, content)
...
async def password_reset_request(
    ...
    content = EmailRawHTMLContent(
        ...
        html_content=f"<p>To reset your password, click the link below:</p><p><a href='{link}'>Reset Password</a></p><p>If you did not request a password reset, please ignore this email.</p><p>This link will expire in 1 hour.</p>",
        sender_name=settings.SENDER_NAME,
    )
    background_tasks.add_task(send_resend_email, recipients, content)
